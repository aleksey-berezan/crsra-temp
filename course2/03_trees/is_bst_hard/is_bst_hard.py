    ...
    def is_bst(self):
        ...
        if self.left != None:
            if not (self.left.data < self.data):
                #print("{0}: false on {1}".format(self.data, 1))
                return (False, None, None)
            left_isbst, max_left, min_right = self.left.is_bst()
            if not left_isbst:
                #print("{0}: false on {1}".format(self.data, 2))
                return (False, None, None)
            if max_left != None and not (max_left < self.data):
                #print("{0}: false on {1}".format(self.data, 3))
                return (False, None, None)

        if self.right != None:
            if not (self.right.data >= self.data):
                #print("{0}: false on {1}".format(self.data, 4))
                return (False, None, None)
            right_isbst, max_left2, min_right2 = self.right.is_bst()
            if not right_isbst:
                #print("{0}: false on {1}".format(self.data, 5))
                return (False, None, None)
            if min_right2 != None and not (min_right2 >= self.data):
                #print("{0}: false on {1}".format(self.data, 6))
                return (False, None, None)
        #print("{0}: true on {1}".format(self.data, 7))
        return (True,
